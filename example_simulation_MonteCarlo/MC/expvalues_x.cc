...
//expvalues_zloops: calculates expectation values of B_p, B_p' and B_p B_p'.
//IN: reads in field configuration 'rbl_z.txt'
//OUT: outputs expectation values 'expv_z.txt'
void expvalues_x(){
...
for (int ne_i=0; ne_i<num_ex; ne_i++){
   ...
   for (int loop_i=0; loop_i<N_loops*2*L*L; loop_i++){   
      ...
      //make loop------- weights[i]=1 if spin i is in loop that is measured, 0 else. Loop contains indices p of plaquettes, for which B_p is part of the Z-loop
      vector<int> loop;
      //initialize weights 
      ...
      //loop: which Bp in loop (make loop) (Z-loop=product of plaquette operators)
      //spin, for which expectation values are measured
      int spin_atlattice=loop_i/3;

      //adjacent plaquettes to spin_atlattice
      int plaquette1,plaquette2;
      get_2plaquettes(spin_atlattice,plaquette1,plaquette2,L);
      
      if (loop_i%3==0)
        loop=make_Bp(plaquette1);
      if (loop_i%3==1)
        loop=make_Bp(plaquette2);
      if (loop_i%3==2)
        loop=make_Bp12(plaquette1,plaquette2);
      
      
      //use the vector 'loop' to obtain 'weights': Flip the spins of a vertex s, if s is an element of 'loop'
      for(int i=0; i<loop.size(); i++){
         flip_plaquette(weights,loop[i],L);
         }
