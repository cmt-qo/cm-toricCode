...
//sign_z: calculates expectation values of sigma_x.
//IN: reads in field configuration 'rbl_z.txt'
//OUT: outputs expectation values 'sign_z.txt'
void sign_z(){
...
//store expectation values of sigma_x for each spin in 'sigmas'
vector<double> sigmas(2*L*L);
   

#pragma omp parallel for num_threads(4)
for (int spin_atlattice=0; spin_atlattice<2*L*L; spin_atlattice++){   //N_loops*2*L*L
